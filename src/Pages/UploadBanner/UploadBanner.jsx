    ...
    <div className="sm:mt-0 mt-10 w-[80%] h-full flex items-center justify-center flex-col">
      ...
      <div className="w-full mt-5 h-fit sm:w-md sm:max-w-md border-2 border-slate-900 flex flex-col px-3 rounded-lg py-8 items-center">
        ...
        <input
          ...
          className="w-full focus:outline-none border border-slate-900 px-2 py-1 rounded-md"
          ref={urlInput}
        />
        {loading ? (
          <Loader />
          ...
          <button
            className="text-sm mt-3 px-3 py-1 bg-slate-800 hover:bg-slate-700 text-white rounded-md"
            onClick={handleBannerUploadFromUnsplash}>
            Submit
          </button>
        )}
        <p className="text-xs italic mt-3">
          *Go to{" "}
          ...
          , search your image, go to that image and click on download button,
          copy the url of the image and paste it in the above box
        </p>
      </div>
    </div>
  );
